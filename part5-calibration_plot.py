...
def run_calibration():
    ...
    model.fit(x,y)
    prob_lr = model.predict_proba(x)[:, 1]

    print("Logistic Regression Calibration Plot")
    calibration_plot(y, prob_lr, n_bins= 5)

    print("Which model is more calibrated?")
    print("Answer: The more calibrated model is the one whose calibration curve stays closer to the 45-degree dashed line.")